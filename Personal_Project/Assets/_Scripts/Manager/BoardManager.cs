...
public class BoardManager : MonoSingleton<BoardManager>
{
	...
	private void Awake()
	{
        ...
        go = Resources.Load(ConstValue.UI_PATH_STAMINA) as GameObject;
        if (go != null)
        {
            DicPrefab.Add(eBoardType.BOARD_STAMINA, go);
        }
        else
    ...
    public BaseBoard AddBoard(BaseObject _keyObject, BaseBoard _board, eBoardType _boardType)
    {
        List<BaseBoard> listBoard = null;

        if (DicBoard.ContainsKey(_keyObject) == false)
        {
            listBoard = new List<BaseBoard>();
            DicBoard.Add(_keyObject, listBoard);
        }
        else
            listBoard = DicBoard[_keyObject];

        if (_board != null)
        {
            listBoard.Add(_board);
        }
